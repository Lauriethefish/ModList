...
MAKE_HOOK_MATCH(MainMenuViewController_DidActivate, &MainMenuViewController::DidActivate, void, MainMenuViewController* self, bool firstActivation, bool addedToHierarchy, bool screenSystemEnabling) {
    ...
    modalView = CreateModal(self->get_transform(), UnityEngine::Vector2(70.0f, 70.0f), [](){
    ...
    VerticalLayoutGroup* layout = CreateVerticalLayoutGroup(modalTransform);
    ...
    TextMeshProUGUI* titleText = CreateText(layoutTransform, failedModsText);
    titleText->set_fontSize(5.0f);
    titleText->set_alignment(TextAlignmentOptions::TopLeft);

    CreateText(layoutTransform, "_______________________________");

    // Add the failed mods to the GUI
    for(std::pair<std::string, std::string> failedMod : failedMods) {
        TextMeshProUGUI* modText = CreateText(layoutTransform, string_format("<color=red>%s</color> didn't load because: \n%s", failedMod.first.c_str(), failedMod.second.c_str()));
        modText->set_overflowMode(TextOverflowModes::Ellipsis);
        modText->set_fontSize(3.5f);

        AddHoverHint(modText->get_gameObject(), failedMod.second); // Show the full fail reason in a hover hint, since there most likely won't be enough space in the modal view
    }
...
extern "C" void load() {
    ...
    BSML::Register::RegisterMainMenu<ModListViewController*>(MOD_ID, "Loaded Mods", "Show Loaded Mods");
    BSML::Register::RegisterSettingsMenu<SettingsViewController*>(MOD_ID, false);
